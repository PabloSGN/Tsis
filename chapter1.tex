...
The OTF describes how different spatial frequencies are transferred from the object to the image, thus providing a comprehensive characterization of the system's ability to resolve fine details. However, since imaging systems measure intensities, we are primarily concerned with how the intensity pattern of an object is transferred to the image. A key metric for quantifying this transfer is modulation, or contrast, which is defined as the ratio between the peaks and valleys of intensity at a given spatial frequency:
...
Another key concepts for assessing the imaging performance are the phase error or wavefront. The wavefront, or phase error of an optical system, is defined as the phase deviation at any point of the image from that of an ideal spherical wavefront \citep{WFE_def}. These deviations arise from a variety of optical imperfections in the imaging system and their effect on the image depends on its nature. Some examples are imperfections of the mirror shape or lenses, that can cause spherical phase aberrations where the PSF is broadened, thus worsening the reolution, or astigmatism, where there is a focus difference between different directions, leading to distorted images; comatic aberrations, or coma, can be caused by a misalignment of optical elements, and create distorted images of sources where they appear to have a tail-like structure. 

One very common metric of the optical quality is th root mean square (rms) of the variance of the wavefront, $\Delta \phi (\xi, \eta)$, usually refered to as RMS wavefront error WFE or simply WFE: 

\begin{equation}
  RMS WFE = \sqrt{\frac{1}{A}\int _ {A} \left( \Delta \phi \right) ^2 \mathrm{d \xi}\mathrm{d \eta}},
\end{equation}
where $A$ is the area of the aperture. 

This value, which is basically the standard deviation of the wavefront across the FoV, is strictly related to beam propagation quality. In fact, in can be shown that the variance of the wavefront can be computed from the Strehl ratio (or viceversa). The Strehl ratio is the ratio between the peak intensity of a point source and the peak ratio of the observed PSF. It is one of the most widely employed metrics to evaluate the optical quality of an optical system. It varies from 1 to 0, where the former corresponds to an ideal unaberrated system, that is, a system operating in the difraction limit. If the aberrations are small, the strehl ratio (St) and the WFE are related through the expression \citep{WFE_def}:

\begin{equation}
  St \simeq \exp \left[ - \left(\frac{2\pi WFE}{\lambda}\right) ^2 \right].
\end{equation}

The wavefront can be represented as a two-dimensional map on the image plane. ETC ETC Zernikes. 

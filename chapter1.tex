...
\subsection{\label{intro - spectros}What do spectropolarimeters tell us about the Sun?}
...
For instance, a straightforward approach to using polarization and spectral data to derive magnetic fields is the center-of-gravity method. As outlined in \cite{utien}, the line-of-sight (LOS) magnetic field strength can be determined using the following expression:
\begin{equation}
  B_{\text{LOS}} = \frac{\lambda _ {+} - \lambda _ -}{2}\frac{4\pi m c}{eg_{L}\lambda_0 ^2}\ \ ,
  \label{eq_spectro: Blos-cog}
\end{equation}  
where $m$ and $e$ are the electron mass and charge in MKS units respectively, $g_L$ stands for the \emph{effective} Landé factor of the line, $\lambda _ 0$ is expressed in nm and $\lambda _ {+}$ and $\lambda _ {-}$ are the centroids of the right and left circularly polarized line components, respectively, and are computed by:
\begin{equation}

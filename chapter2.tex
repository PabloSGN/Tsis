...
The imaging E2E tests involved projecting several targets at the F4 focus, including a USAF test target, star targets, and a grid, observed both with and without the PD plate. These targets were utilized to evaluate the MTF and to assess the resolving power of TuMag. The PD measurements enabled verification of the wavefront error (WFE) derived from the MTF and an evaluation of the image quality following image restoration. The MTF functions (see Figure XXX) indicate that the performance of the 517 nm pre-filter and the horizontal behavior of the 525 nm pre-filters align well with expectations, corresponding to WFE values better than $\lambda /10$. However, the vertical performance of the iron pre-filters appears to be degraded. This observed astigmatism is attributed to the etalon and physical deformations of the pre-filters caused by the mechanical method used to secure and tilt them. This effect is particularly noticeable in the iron pre-filters due to the higher angles of incidence required for their tuning.

...
def change_names(dir_d,light,cond,date,temp):
    ...
    for gain in gainl:
        if gain=='1':
            time=np.arange(1,485,16) #IN TLINES UNITS!!
        if gain=='2':
            time=np.arange(1,362,12) #IN TLINES UNITS!!
        if gain=='4':
            time=np.arange(1,752,25) #IN TLINES UNITS!!
...
def read_data(name,gain,bl,Tset,mode,Nx=2048,Ny=2048,limit_frames=0,time_format='float'):
    dir = './'
...
def getHeader( pathFullImage ):
    #global fullReceivedImage
    #global receivedHeader
    ...
    #print("Total Length (from MPS Header)-> {}".format(totalLength))
    #print("Total Length (file read by Python)-> {}".format(len(fullReceivedImage)))

    receivedHeader = fullReceivedImage[0:headerLength]
    receivedImage = fullReceivedImage[headerLength:totalLength-nBytesLengthTail]

    #print("Header Length -> {}".format(len(receivedHeader)))
    #print("Image Length -> {}".format(len(receivedImage)))

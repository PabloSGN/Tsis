...
Despite the improvement in optical performance, the telecentric configuration is no exempt of challenges. In this configuration each light ray trasverses the etalon through different regions, thus being affected differently at each point from the various small scale defects present in the etalon. This defects, compose the so-called, cavity-map and its correction can be one of the main challenges of employing this setups.
...
The imaging response of the instrument coincides with the PSF of the instrument when the optical response is invariant against translations. In such a case, we can express the imaging response as: $S =\mathcal{S}(\left(\xi - \xi_0 , \eta - \eta_0; \lambda-\lambda_{s}\right)) $, and alsosubstitute the last two integrals by the convolution operator.  However, this is not strictly true in etalon-based instruments, where the response varies pixel to pixel either because of etalon irregularities or because of variations in the illumination across its clear aperture.

Deriving S requires determining the electric field of the polychromatic wave in the image plane. This field can be calculated by Fourier transforming the electric field $(E^{(t)})$ across the pupil, such that the eletric field at any point $(\xi, \eta)$ of the image plane follows the expression \citep{franI}:
...
It is possible to recast eq. \eqref{eq_etalon_theory: electric_image_plane} in terms of the radial coordinates of the pupil and anallytically solve the equations. The resulting spectral transmission profile of an ideal telecentric etalon  is given by \citep{franIV}:
\begin{equation}
...
The target of the observations is a MHD simulation of a sunspot taken from \cite{mancha-seminal} where several simualations are carried out employing the MURaM code \citep{MURaM}. For this work we have employed a subset of the original data of 561x561 pixeles. The original size of the data cube is 1536x128x1536 pixels with a grid spacing of 32 km in the horizontal direction and 16 km in the vertical. The spectral profiles were employed to synthesize the Stokes profiles performing an inversion using the SIR code \citep{sir} with a spectral sampling of 10m\r{A}/pix over a spectral range that goes from -500 to 1500 m\r{A}  from the rest wavelength of the line core of the Fe I 6173\r{A}. For a detailed description of the Stokes synthesis we refer the reader to \cite{mancha-carlos}.

As explained in previous chapters, tunable spectropolarimeters do not measure the stokes components directly, but rather infer it from a series of independent measurements. In order to replicate this behaviour, we generated four modulations from the MHD simulations employing an ideal modulation matrix. The demodulation of the images is performed by applying an ideal demodulation matrix computed by inverting the modulating matrix.
...
In order to replicate the behaviour of real instrumentation we have introduced a gain variation and cavity errors. The former is straightforward to introduce as it is considered independently in Eq-~\eqref{eq_mancha: Intensity} as a pixel-dependent multiplicative factor that affects the whole mesurement, $g$. In contrast, the effects of the cavity map are more cumbersome. Pixel-to-pixel variations in etalon defects shift the transmission profile of the FPI and must be accounted for when computing the imaging response. In contrast, the effects of the cavity map are more convoluted. Pixel-to-pixel variations in etalon defects shift the transmission profile of the FPI and must be accounted for when computing the imaging response. Both the gain and cavity map introduced in the simulations have been selected to resemble the real-case scenario of the SO/PHI instrument. The gain map (Fig. ~\ref{fig_mancha: Inputs} top right panel) utilized in the simulations is derived from a flat field of the SO/PHI-HRT instrument, and the cavity map employed (Fig. ~\ref{fig_mancha: Inputs} top central panel) is sourced from the cavity map of PHI's etalon.

In addition to the inclusion of gain and cavity errors, a series of flat-field observations need to be also generated for the posterior correction. As with real instruments, flat-fields are but another set of observations, usually with the same properties regarding spectral sampling and modulation scheme, but with a different target, namely, a homogeneous and structure-free region. The strategy we have followed to generate this observations is to apply the same observation procedure of the sunspot observations to a generated \textit{ideal} flat region. This new target has been generated by computing the average spectral profile of a region of quiet Sun from the simulation, and replicating this profile over the whole FoV. This way, we generate an ideal flat-observation, completely homogeneous, where every pixel repropduces the average spectral profile of the quiet Sun. 

Since the main goal of these tests is to assess the impact of incomplete corrections of the cavity map, we conducted these simulations under two scenarios: one incorporating the cavity error and another assuming a defect-free etalon. The latter will serve us as a reference of an observation devoid from any etalon-induced defect, but with identical spectral and spatial degradations. 
...
In some pipelines, the problems of the cavity-map are adressed trying to "deconvolving" in some way the effect of the etalon's transmission profile from the flat-field observations. However, it is common practice to neglect the asymmetric nature of real telecentric mounts due to the complex analytical equations it invovles, and the potential effects of profile asymmetries are typically assumed to be negligible. 
...
In an attempt to quantify the error of this assumptions, we now compare the results obtained with the assymetric and symmetric profiles. The main effect of the assymetry is a systematic shift of the profile measurements towards the blue wing of the spectral profile. This shift results in an average difference of around $-0.2$ km s$^{-1}$ that can be seen over the whole FoV. 

Nevertheless, this systematic shift can be removed from the velocities computed with the assymetric FPI by substracting the average difference between symmetric and assymetric velocities. Figure \ref{fig_mancha: vlos_asym_vs_sym} depicts the differences in the computed velocities for the symmetric and assymetric FPIs, after applying this correction. Despite accounting for the systematic shift, discrepancies are still present, predominantly within the umbra, although they are observed across the entire FoV. The umbra is again more sensible than other regions, with velocity discrepancies higher than 100 m s$^{-1}$ thorughout the majority of the umbra, and exibiting a global shift towards smaller velocities for the assymetric scenario. Outside the umbra, differences are smaller but still prevalent throughout the entire FoV. 

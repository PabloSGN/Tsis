...
It is easy to demonstrate \citep{franI} that the spectral transmission profile of an ideal collimated etalon tuned at wavelength $\lambda _ s$ takes the following form:
\begin{equation}
...
The spatial dependence of $\psi$ arises naturally as $\theta$ changes from pixel to pixel. In fact, the spectral behaviour of the transmission prodile is encoded in the parameter $a_s(\lambda,\theta)$, which is a function of the refractive index of the etalon cavity, $n$; the distance between mirrors, $d$; and the angle of the incident beam, $\theta$. The reflectivity $R$ of the mirrors determines the width of the resonance peadfsaks through the parameter $F$, $F \equiv 4R (1 - R )^{-2}$.

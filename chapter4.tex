...
\begin{equation}
    ...
    \label{eq_etalon_theory: General}
\end{equation}
...
However, in an ideal collimated etalon $delta$ loses its dependence on the coordinates and takes the following shape:
\begin{equation}
  \delta (\xi, \eta, \lambda) = \frac{4\pi d}{\lambda}\sqrt{n ^{\prime 2}  - n ^2 + \frac{(nf) ^2}{\xi ^2 + \eta ^2 + f ^2}}.
  \label{eq_etalon_theory: collimated_delta}
\end{equation}

In such a case, it can be shown that the local PSF of the etalon tuned at the wavelength $\lambda _ s$ follows the expression:

\begin{equation}
  \mathcal{S}\left(\xi_0, \eta_0; \xi , \eta; \lambda _ s\right) = \frac{\tau}{1 + F \sin ^2 (\delta(\xi_0, \eta_0, \lambda _s) / 2)}\left( \frac{2 J_1 (\delta(\xi, \eta, \lambda _s) - \delta(\xi _ 0, \eta _ 0, \lambda _s)) }{\delta(\xi, \eta, \lambda _s) - \delta(\xi _ 0, \eta _ 0, \lambda _s)}\right) ^2,
\end{equation}
where the variable $z$ is defined as:
\begin{equation}
  z = \frac{2 \pi}{\lambda}R_{pup}\frac{\sqrt{\eta ^2 + \nu ^2}}{f}
\end{equation}


The spatial dependence of $\psi$ arises naturally as $\theta$ changes from pixel to pixel. In fact, the spectral behaviour of the transmission profile is encoded in the parameter $a_s(\lambda,\theta)$, which is a function of the refractive index of the etalon cavity, $n$; the distance between mirrors, $d$; and the angle of the incident beam, $\theta$. The reflectivity $R$ of the mirrors determines the width of the resonance peadfsaks through the parameter $F$, $F \equiv 4R (1 - R )^{-2}$.

Local defects in the collimated configuration are averaged out, which means that $d$ and $n$ respectively represent the mean values of the thickness and refractive index across the clear aperture of the FPI, and thus remain constant for every pixel. Yet, they produce a broadening of the transmission profile and worsen the optical quality of the instrument. 

Analytical solutions of equation \eqref{eq_etalon_theory: electric_image_plane} are imposible to obtain if $\delta$ has a dependence on the pupil coordinates, as is the case of etalon with a presence of local defects. In that case, the transmssion profile of the etalon has to be evaluated numerically. 


\textcolor{red}{Assessing the spatial PSF of the FPI is more challenging, as it can only be determined analytically for monochromatic light and in the absence of defects. In all other cases, the PSF must be evaluated numerically due to the lack of analytical solutions to the integrals. We will not delve into the equations as it lies beyond the scope of this thesis. However, interested readers are referred to the work of \cite{franI}, where this topic is extensively discussed.}
